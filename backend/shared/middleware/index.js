...
const validateContentType = (allowedTypes) => {
    return (req, res, next) => {
        ...
        if (!contentType) {
            return (0, errors_1.sendError)(res, 'Content-Type header is required', 400);
        }
        const isAllowed = allowedTypes.some(type => contentType.includes(type));
        if (!isAllowed) {
            return (0, errors_1.sendError)(res, `Invalid Content-Type. Allowed types: ${allowedTypes.join(', ')}`, 400);
        }
        next();
    };
};
...
const validateApiKey = (req, res, next) => {
    ...
    if (!apiKey) {
        return (0, errors_1.sendError)(res, 'API Key required', 401);
    }
    if (!validApiKeys.includes(apiKey)) {
        return (0, errors_1.sendError)(res, 'Invalid API Key', 401);
    }
    next();
};
...
const validateBarId = (req, res, next) => {
    ...
    if (!barId) {
        return (0, errors_1.sendError)(res, 'Bar ID is required', 400);
    }
    // Validate ObjectId format
    if (!mongoose_1.default.Types.ObjectId.isValid(barId)) {
        return (0, errors_1.sendError)(res, 'Invalid Bar ID format', 400);
    }
...
const validateTableId = (req, res, next) => {
    ...
    if (!tableId) {
        return (0, errors_1.sendError)(res, 'Table ID is required', 400);
    }
    // Validar formato UUID
    const uuidRegex = /^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i;
    if (!uuidRegex.test(tableId)) {
        return (0, errors_1.sendError)(res, 'Invalid Table ID format', 400);
    }
...
const paginationMiddleware = (req, res, next) => {
    ...
    const limit = Math.min(parseInt(req.query.limit) || 10, 100); // Máximo 100 items
...
const validateJsonMiddleware = (req, res, next) => {
    ...
    if (contentType && contentType.includes('application/json')) {
        if (Object.keys(req.body).length === 0) {
            return (0, errors_1.sendError)(res, 'Request body cannot be empty', 400);
        }
    }
    next();
};
