...
interface QRGeneratorSimpleProps {
  barId: string;
  barName?: string;
...
export default function QRGeneratorSimple({ 
  barId, 
  barName = 'Encore Bar',
  onGenerate,
  maxTables = 50 
}: QRGeneratorSimpleProps) {
  ...
  const downloadQRAsPNG = (qr: QRData) => {
    ...
    // Dibujar placeholder QR (cuadrado con texto)
    ...
    ctx.font = '12px Arial';
    ctx.fillText(`Table ${qr.tableNumber}`, canvas.width / 2, qrY + qrSize / 2 + 20);
    ctx.fillText('Scan to connect', canvas.width / 2, qrY + qrSize / 2 + 40);
  ...
  // Renderizar QR placeholder
  const renderQRPlaceholder = (qr: QRData, size: number = 200) => (
    <div 
      className="bg-gray-100 border-2 border-gray-300 rounded-lg flex items-center justify-center"
      style={{ width: size, height: size }}
    >
      <div className="text-center p-2">
        <QrCodeIcon className="w-8 h-8 mx-auto mb-2 text-gray-400" />
        <div className="text-xs font-semibold text-gray-600">Mesa {qr.tableNumber}</div>
            ...
            <div className="p-4 border rounded-lg">
              <div className="text-center">
                ...
                <div className="inline-block p-4 bg-white rounded-lg border">
                  {renderQRPlaceholder(generatedQRs.find(qr => qr.tableNumber === selectedTable)!, qrConfig.size)}
                </div>
              </div>
            </div>
          )}
